  ...
  <div style="width: 900px;">
        ...
        <v-select
          ...
          :options="
            $getGrid('locationInventory')
              .filter((inv) => inv.part === rowData.part && inv.quantityOnHand )
              .map((inv) => ({ 
                 title: `${inv.combinedCode||''} - ${inv.quantityOnHand}`,
                 code: inv.rowKey }))
          "
          label="title"
    ...
    >
      ...
      <v-select
        ...
        :options="
          $getGrid('locationInventory')
            .filter((inv) => inv.part === rowData.part &&  rowData.part && inv.quantityOnHand )
            .map((inv) => ({ 
                 title: `${inv.combinedCode||''} - ${inv.quantityOnHand}`,
            code: inv.rowKey }))
        "
